...
//			}
	    ...
	    $(".global").on("click",".box ul a",function(){
	    	...
	    	setDomainCookie("stype",c_stype,".zhibo8.cc");
	    ...
	    $(window).on("scroll",function(){
	    	...
	    	if(top-scrollH<=0){
	    		...
	    		$(document.body).hasClass("down_fixed") && $nav.addClass("down_nav_fixed");
	    	}else{
	    		$nav.removeClass("nav_fixed").next(".empty").hide();
	    		$nav.hasClass("down__nav_fixed") && $nav.removeClass("down_nav_fixed");
	    	}
	    });
	    /*-----------------重要、篮球、足球、其他、全部的选择--------------------*/
	   /*-----android,uc的判断--------*/
...
//	        	$("body").css("margin-top",'40px');
	        	$("#m_a_d_v").show();
	        	$nav.addClass("down_nav_fixed");
	        }
	        $("#m_close").click(function(){
	            $("#m_a_d_v").hide();
	            setCookie("m_adv",1);
	            $(document.body).removeClass("down_fixed");
	            $nav.hasClass("down_nav_fixed") && $nav.removeClass("down_nav_fixed");
				return false;
	        });
	        $("#m_adv > a").click(function(){
	        	
	            window.location = '//m.zhibo8.cc/download/';//'<?php echo $apk_url; ?>';
	        });
	    ...
	    $(".jump").click(function(){
	        setDomainCookie("defaultJumpDomain","www",".zhibo8.cc");
	    });
	    /*-----android,uc的判断--------*/
	   /*--------------点击返回顶部，系统的判断------------*/
